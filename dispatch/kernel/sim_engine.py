"""
Simulation engine — deterministic full-window re-simulation.

Moved from ``services/sim_engine.py`` with one behavioural addition: the cost
model is now a parameter instead of module constants, which is what makes cost
sensitivity analysis possible ("how high must costs go before alpha dies").
Defaults reproduce the previous hardcoded values exactly, so production output
is unchanged.

Each run re-simulates from the window start using the price pivot and
pre-computed signals. No incremental state patching: idempotent, drift-free,
safe to re-run any time.

Two position-sizing modes are simulated for every strategy:
...
Execution semantics:
  - sell signals execute the day AFTER the signal (T+1 at close)
  - every RB-th trading day is a rebalance day: liquidate, then buy per the
    previous day's signals
  - costs applied on both sides; stamp duty on sells only
"""
...
class CostModel:
    """Transaction costs. Defaults match the original hardcoded constants."""
    commission: float = 0.001
    slippage: float = 0.001
    stamp_duty: float = 0.001

    def scaled(self, factor: float) -> "CostModel":
        """Uniformly scaled copy — the knob for cost sensitivity curves."""
        return CostModel(self.commission * factor,
                         self.slippage * factor,
                         self.stamp_duty * factor)
    ...
    def round_trip_bps(self) -> float:
        """Approximate all-in round-trip cost in basis points."""
        return (self.commission * 2 + self.slippage * 2 + self.stamp_duty) * 10_000


DEFAULT_COSTS = CostModel()

# module-level aliases kept so existing imports of the old constants still work
COMMISSION = DEFAULT_COSTS.commission
SLIPPAGE = DEFAULT_COSTS.slippage
STAMP_DUTY = DEFAULT_COSTS.stamp_duty
...
def simulate_strategy(pivot: pd.DataFrame, signals: dict, strategy: str,
                      ...
                      rebalance_days: int = RB, max_positions: int = MAX_POS,
                      costs: CostModel | None = None,
                      modes: tuple[str, ...] = MODES,
                      initial_cash: float = INITIAL_CASH) -> dict:
    """Run the requested modes for one strategy over sim_dates.

    rebalance_days / max_positions normally come from the strategy's registry
    spec, so daily-rebalancing strategies coexist with monthly ones.

    Returns {mode: {"snapshots": [...], "trades": [...], "costs": {...},
                    "final_equity": float, "positions": {...}, "cash": float}}
    """
    cm = costs or DEFAULT_COSTS
    commission, slippage, stamp_duty = cm.commission, cm.slippage, cm.stamp_duty

    all_dates = list(pivot.index)
    date_pos = {d: i for i, d in enumerate(all_dates)}
    mvol = _market_vol_series(pivot) if "adaptive" in modes else None
    ...
    for mode in modes:
        ...
        for dt in sim_dates:
            ...
            if is_rb and buys:
                ...
                if valid:
                    if mode == "adaptive" and mvol is not None:
...
def summary_metrics(snaps: list[dict]) -> dict:
    """Legacy 7-metric summary. Kept for the production pipeline and emails.

    New code should prefer ``research.metrics.full_metrics`` which is a superset.
    """
    if len(snaps) < 2:
